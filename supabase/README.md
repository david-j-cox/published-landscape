...
2. In the SQL Editor, run the files in `migrations/` in order (0001 through 0005) - paste and run each one. Paste from the clipboard rather than copying out of a terminal; long statements pick up dropped characters at wrap points and fail with confusing syntax errors.
...
5. From then on, manage people from `/admin` in the app. Only invited emails can sign in; sign-up is disabled.
